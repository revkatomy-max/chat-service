...
-- // CEK CRYSTAL RESPAWN //
local function CheckCrystalSpawn(rawMsg)
    if not SCRIPT_ACTIVE then return end
    local lower = string.lower(rawMsg)
    -- Tangkap berbagai variasi pesan crystal respawn di chat server
    if not (string.find(lower, "crystal") and (
        string.find(lower, "respawn") or
        string.find(lower, "spawn") or
        string.find(lower, "muncul") or
        string.find(lower, "appeared") or
        string.find(lower, "has spawned") or
        string.find(lower, "telah muncul")
    )) then return end

    -- Coba ekstrak nama crystal dari pesan
    local crystalName = rawMsg:match("[Aa] (.+) has spawned") or
                        rawMsg:match("[Aa] (.+) respawned") or
                        rawMsg:match("[Cc]rystal (.+) muncul") or
                        rawMsg:match("([Cc]rystal[%a%s]*) telah muncul") or
                        "Crystal"

    crystalName = crystalName:match("^%s*(.-)%s*$") or crystalName

    SendFishWebhook("💎 CRYSTAL RESPAWN!", nil, 3447003, {
        {["name"] = "🔮 Crystal",  ["value"] = "**" .. crystalName .. "**",           ["inline"] = true},
        {["name"] = "📢 Info",     ["value"] = "Crystal telah respawn di server!",    ["inline"] = true},
        {["name"] = "💬 Pesan",    ["value"] = "`" .. rawMsg:match("^%s*(.-)%s*$") .. "`", ["inline"] = false},
    }, nil, nil, "")
end
